...
const generate_payment_link = (e) => {
  ...
  if (isNotLogin == "True") {
    full_name = frappe.utils.xss_sanitise(
      ($("#signup_name").val() || "").trim()
    );
    email = frappe.utils.xss_sanitise(($("#signup_email").val() || "").trim());
    mobile_no = frappe.utils.xss_sanitise(
      ($("#signup_phone").val() || "").trim()
    );
    frappe.call({
      ...
      callback: (data) => {
        if (data.message && data.message[0] == 1) {
          ...
          if (
            data.message[1] ==
            "Already Registered. Please click Login below to login"
          ) {
...
const enrolling_course = (doctype, docname, mobile_no, full_name, country) => {
  ...
  frappe.call({
    ...
    method: "erptech_lms.erptech_lms.utils.get_payment_options_instamojo",
    args: {
      doctype: doctype,
      docname: docname,
      phone: mobile_no,
    ...
    callback: (data) => {
      ...
      Instamojo.open(
        `https://test.instamojo.com/@sumit_fb76c/?embed=form&purpose=${options.purpose}&amount=${options.amount}&name=John+Doe&email=johndoe@example.com&phone=1234567890`,
        options
      );
